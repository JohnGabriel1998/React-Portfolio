...
function createTextTexture(gl: WebGLRenderingContext, text: string, font = 'bold 30px monospace', color = 'black') {
  if (!text || text.trim() === '') return { texture: null, width: 0, height: 0 };
  
  const canvas = document.createElement('canvas');
  const context = canvas.getContext('2d');
  if (!context) return { texture: null, width: 0, height: 0 };
  
  // Ensure minimum canvas size for better rendering
  ...
  // Use power-of-2 dimensions for better WebGL compatibility with more padding
  canvas.width = Math.pow(2, Math.ceil(Math.log2(Math.max(textWidth + 100, 1024))));
  canvas.height = Math.pow(2, Math.ceil(Math.log2(Math.max(textHeight + 100, 1024))));
  
  // Clear and set up context again after resize
  context.font = font;
  ...
  // Add text shadow for better visibility (lighter shadow for dark text on light background)
  const isDarkText = color === '#1f2937' || color === '#000000' || color.startsWith('#1') || color.startsWith('#0');
  if (isDarkText) {
    // Light shadow for dark text - subtle for readability
  ...
  // Draw text with thicker stroke for better readability
  if (isDarkText) {
  ...
  // Fill text with color - make it bold and clear
  context.fillStyle = color;
...
class Title {
  ...
  createMesh() {
    ...
    if (texture.image && (texture.image as HTMLCanvasElement).width > 0 && (texture.image as HTMLCanvasElement).height > 0) {
      texture.needsUpdate = true;
    }
    ...
    // Position text below the card with more spacing
    this.mesh.position.y = -this.plane.scale.y * 0.5 - textHeight * 0.5 - 0.22;
...
class Media {
  ...
  update(scroll: { current: number; last: number }, direction: string) {
    ...
    if (this.bend === 0) {
      ...
      // Add slight tilt rotation for visual interest
      this.plane.rotation.z = Math.sin(x * 0.01) * 0.05;
    } else {
      ...
      if (this.bend > 0) {
        ...
        // Add slight tilt to the rotation
        this.plane.rotation.z = -Math.sign(x) * Math.asin(effectiveX / R) + Math.sin(x * 0.01) * 0.03;
      } else {
        this.plane.position.y = arc;
        this.plane.rotation.z = Math.sign(x) * Math.asin(effectiveX / R) + Math.sin(x * 0.01) * 0.03;
      }
  ...
  onResize({ screen, viewport }: { screen?: { width: number; height: number }; viewport?: { width: number; height: number } } = {}) {
    ...
    // Make cards more rectangular/landscape oriented to match image style
    ...
    // Increase padding between cards for better spacing
    this.padding = 2.5;
